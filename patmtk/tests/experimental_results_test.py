...
@pytest.fixture(scope='module')
def kernel_data_0():
    return [[1,2], [3,4], [5,6], [120, 100], {'t01': {'coherence': [1,2,3],
                                                             'contrast': [6, 3],
                                                             'purity': [1, 8]},
...
def exp_res_obj1(kernel_data_0, kernel_data_1, json_path, test_collection_dir):
    ...
    #            'topic-kernel-0.6': kernel_data_0,
    #            'topic-kernel-0.8': kernel_data_1,
    ...
    exp = ExperimentalResults('dir', 'gav', 5, 2, ['t0', 't1'], ['t2', 't3', 't4'], {'dcn': 1, 'icn': 5},
                               {'perplexity': [1, 2, 3],
                                'sparsity-phi-@dc': [-2, -4, -6],
                                'sparsity-phi-@ic': [-56, -12, -32],
                                'sparsity-theta': [2, 4, 6],
                                ...
                                'topic-kernel-0.6': kernel_data_0,
                                'topic-kernel-0.8': kernel_data_1,
                                'top-tokens-10': [[1, 2], {'t01': [12, 22, 3], 't00': [10, 2, 3], 't02': [10, 11]}],
                                'top-tokens-100': [[10, 20], {'t01': [5, 7, 9], 't00': [12, 32, 3], 't02': [11, 1]}],
                                'tau-trajectories': {'phi': [1, 2, 3, 4, 5], 'theta': [5, 6, 7, 8, 9]},
                                'collection_passes': [3]},
                               {'topic-kernel-0.6': {'t00': ['a', 'b', 'c'],
                                                     't01': ['d', 'e', 'f'],
                                                     't02': ['g', 'h', 'i']},
                                'topic-kernel-0.8': {'t00': ['j', 'k', 'l'],
                                                     't01': ['m', 'n', 'o'],
                                                     't02': ['p', 'q', 'r']}}, {'top-tokens-10': {'t00': ['s', 't', 'u'],
                                    't01': ['v', 'x', 'y'],
                                    't02': ['z', 'a1', 'b1']},
                  'top-tokens-100': {'t00': ['c1', 'd1', 'e1'],
                                     't01': ['f1', 'g1', 'h1'],
...
class TestExperimentalResults():
    ...
    def test_creation_from_experiment(self, trained_model_n_experiment, train_settings):
        ...
        col_passes = int(train_settings['learning']['collection_passes'])

        assert hasattr(exp_res, 'tracked')
        assert hasattr(exp_res, 'scalars')
        assert exp_res.scalars.nb_topics == int(train_settings['learning']['nb_topics'])
        assert exp_res.scalars.model_label == model.label
        assert all(map(lambda x: len(x) == col_passes, [exp_res.tracked.perplexity, exp_res.tracked.sparsity_theta, exp_res.tracked.sparsity_phi_d, exp_res.tracked.sparsity_phi_i]))
        for reg_def in (_ for _ in train_settings['scores'] if _.startswith('topic-kernel-')):
            tr_kernel = getattr(exp_res.tracked, 'kernel'+reg_def.split('-')[-1][2:])
            assert all(map(lambda x: len(getattr(tr_kernel.average, x)) == col_passes, ['coherence', 'contrast', 'purity']))
        assert all(map(lambda x: len(x.average_coherence) == col_passes,
                       (getattr(exp_res.tracked, 'top' + _.split('-')[-1]) for _ in train_settings['scores'] if _.startswith('top-tokens-'))))
        assert all(map(lambda x: len(x.all) == col_passes,
                       (getattr(exp_res.tracked, 'sparsity_phi_' + _.split('-')[-1][1]) for _ in train_settings['scores'] if
                        _.startswith('sparsity-phi-@'))))

        for reg_def in (_ for _ in train_settings['scores'] if _.startswith('top-tokens-')):
        ...
        for kernel, kernel_def in zip(exp_res.final.kernels, exp_res.final.kernel_defs):
            for topic_name in exp_res.scalars.domain_topics:
                # print dir(getattr(exp_res.final, kernel))
                # print getattr(getattr(exp_res.final, kernel), topic_name).tokens
                ...
                assert getattr(getattr(exp_res.final, kernel), topic_name).tokens == experiment.topic_model.artm_model.score_tracker[experiment.topic_model.definition2evaluator_name[kernel_def]].tokens[-1][topic_name]
    ...
    def _assert_experimental_results_creation(self, exp_obj):
        ...
        assert exp_obj.tracked.top10.average_coherence.all == [1, 2]

        assert exp_obj.tracked.tau_trajectories.phi.all == [1, 2, 3, 4, 5]
        assert exp_obj.tracked.tau_trajectories.theta.last == 9
        ...
        assert exp_obj.final.kernels == ['kernel6', 'kernel8']
