...
if __name__ == '__main__':
    ...
    tuning_definition = tdb.initialize().nb_topics([10,12]).collection_passes(50).document_passes(1).background_topics_pct([0.2]).\
        ideology_class_weight([10]). \
        sparse_phi().deactivate(5).kind(['linear']).start(-10).end([-100, -200]). \
        sparse_theta().deactivate(5).kind('linear').start([-30]).end([-50, -150]).build()
    ...
    tuner.tune(tuning_definition, prefix_label=args.prefix, append_explorables='all', append_static=True, force_overwrite=args.overwrite)
...
#     return tuner
    ...
    # from patm.tuning.building import tuner_definition_builder as tdb
    #
    # tuning_definition = tdb.initialize().nb_topics([20]).collection_passes(100).document_passes(1).background_topics_pct(
    ...
    # #                                      'sparse-theta': {'alpha_iter': 1}}
    # # 'sparse-theta': {'alpha_iter': 'linear_1_4'}}
    # tuner.tune(tuning_definition, prefix_label='tag1', append_explorables='all', append_static=True)
