...
class Tuner(object):
    ...
    """
    def __init__(self, collection, train_config, reg_cfg, static_parameters, explorable_parameters, model_type=None):
        """
    ...
    def _vector_gen(self):

    def _get_label(self):
        _ =  self._label + '_' +  '_'.join([self.settings['learning'][_] for _ in self._static_parameters])
        if _[-1] == '_':
            return _[:-1]

    def _create_model(self, label):
        self._tm, _ = self.trainer.model_factory.create_model(label, self._val('nb_topics'), self._train_cfg, self._reg_cfg)
        self._tm.set_document_passes = self._val('document_passes')
    ...
    def tune(self, search='grid-search'):
        for self.parameter_vector in self._vector_gen[search]:
            print self.parameter_vector
            # self._empty_parameter_space()
            # cur_label = self._get_label()
...
def get_cli_arguments():
    ...
    parser.add_argument('train-cfg', dest='train_cfg', help='the fixed model parameters and regularizers to use. Regularizers can be turned on/off after consecutive train iterations through dynamic tau trajectory, but no new regularizer can be added after initialization')
...
if __name__ == '__main__':
    ...
    label = 'plsa'
    static = {'document_passes': 100}
    explorable = {'nb_topics': nb_topics,
                  'document_passes': doc_iterations}

    tuner = Tuner(args.dataset, args.train_cfg, regularizers_cfg, static, map(lambda x: (x, explorables[x]), sorted(explorable.keys())), model_type=label)
    tuner.tune()
