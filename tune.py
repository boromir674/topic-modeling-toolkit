...
class Tuner(object):
    ...
    def __init__(self, collection, train_config, regularizers_cfg, static_parameters, explorable_parameters, prefix_label=None, append_explorables='all', append_static=True):
        ...
        if len(ses) == 1 and type(ses[0]) == list and type(ses[0][0]) == list:
    ...
    def _val(self, parameter_name):
        ...
        if parameter_name in self.explorable_parameters:
            print 'L', len(self.parameter_vector), self.parameter_vector
            return {'deactivation_period_pct': self.parameter_vector[self.explorable_parameters.index(parameter_name)+self._traj_attr2index['deactivation_period_pct']],
                    'start': self.parameter_vector[self.explorable_parameters.index(parameter_name)+self._traj_attr2index['start']],
                    'end': self.parameter_vector[self.explorable_parameters.index(parameter_name)+self._traj_attr2index['end']]}

    def _build_traj(self, traj_def, total_iters):
        deactivation_span = int(traj_def['deactivation_period_pct'] * total_iters / 100.0)
...
if __name__ == '__main__':
    ...
    #####################
    # REGULARIZERS POOL
    traj1 = {'deactivation_period_pct': 10, 'start': -0.1, 'end': -18}
    tr2 = {'deactivation_period_pct': 10, 'start': -0.2, 'end': -12}
    ...
    tr3 = {'deactivation_period_pct': [10], 'start': [-0.2, -0.4], 'end': [-5]}
    tr4 = {'deactivation_period_pct': [10], 'start': [-0.1, -0.3, -0.5], 'end': [-1, -3, -5]}
    tr4 = {'deactivation_period_pct': [10], 'start': [-0.2, -0.5, -0.8], 'end': [-1, -4, -7]}
    ##############################
    ...
    tuner = Tuner(args.dataset, args.train_cfg, reg_cfg,
                  ...
                  [('nb_topics', [20, 40, 60, 80, 100]), ('document_passes', [5, 10, 15])],
                  prefix_label='plsa+smt+smp',
                  append_explorables='all',
                  append_static=True)
    tuner.tune([1, {'sparse-theta': {'name': 'spd_t', 'tau': -0.1}, 'smooth-theta': {'name': 'smb_t', 'tau': 1},
                            'sparse-phi': {'name': 'spd_p', 'tau': -0.1}, 'smooth-phi': {'name': 'smb_p', 'tau': 1}}])
